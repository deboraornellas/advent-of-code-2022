...
export const day17 = (directions: string[], numberOfRounds?: number) => {
  let pos = [3, 2];
  let shapeIndex = 0;
  ...
  while (round < (numberOfRounds || 2022)) {
    ...
    //   '->',
    //   newPos,
    //   moveVertically ? 'vertical' : 'horizontal',
    //   !moveVertically ? directions[directionIndex] : '',
    //   shapes[shapeIndex],
    //   // stoppedRockPositions,
    //   round,
    // );
  ...
  // console.log(stoppedRockPositions);
  return (
    stoppedRockPositions.reduce((acc, curr) => {
